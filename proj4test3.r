...
dummy7<-function(theta){ # This is positive definite
  x<-theta[1];y<-theta[2];z<-theta[3]
  return(70*(x^2+y^2+z^2)+2*x*z+4*y*z)
}
...
newt(c(10,10,10),dummy5,dummy_grad5, maxit=500) # This doesn't match with nlm

newt(c(10,10,10),dummy6,dummy_grad6, maxit=500) # This doesn't match with nlm

newt(c(10,10,10),dummy7,dummy_grad7, maxit=500) # This converges in 2 iterations in newt(), 
# although the outputs do not match up with nlm

# !!!!!This finishes in 2 iterations in nlm but newt() returns max.half warning!!!!!
f<-function(x){(x[1]+x[2]+x[3])^2-1}   # dummy6
p=c(10,10,10)
nlm(f,p)